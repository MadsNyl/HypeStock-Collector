...
for analytic in analytics:
    ...
    _, last_price, min_price, max_price, volume, timing = get_company_name(analytic[0])
    if not timing: continue
    timing = timing[-10:]
    timing = f"{timing[-4:]}-{timing[-10:-8]}-{timing[-7:-5]}"
    date_check = check_date(timing, analytic[0])   
    if date_check:
        data.append((analytic[0], float(last_price.replace(",", "")[1:]), float(min_price.replace(",", "")[1:]), float(max_price.replace(",", "")[1:]), int(volume.replace(",", "")), timing))
